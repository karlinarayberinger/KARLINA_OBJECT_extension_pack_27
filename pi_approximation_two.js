...
/**
 * Retrieve the number which is enclosed inside of the HTML span element whose id is "seconds_elapsed_span" on the corresponding web page.
 * 
 * @return {Number} an integer which is assumed to be no smaller than 0.
 */
function get_seconds_elapsed() {
	let seconds_elapsed_span = undefined, seconds_remaining = 0;
	try {
		seconds_elapsed_span = document.getElementById("seconds_elapsed_span");
		seconds_elapsed = parseInt(seconds_elapsed_span.innerHTML);
		if (seconds_elapsed < 0) throw "seconds_elapsed must be an integer no smaller than 0.";
		return seconds_elapsed;
	}
	catch(exception) {
		console.log("An exception to expected functioning occurred in get_seconds_elapsed(): " + exception);
	}
}

/**
 * Add one to the total number of seconds elapsed (which is displayed inside the HTML span element whose id is "seconds_elapsed_span" on the corresponding web page).
 * 
 * Assume that the number of seconds remaining is always an integer which is no smaller than 0.
 */
function increment_seconds_elapsed() {
	let seconds_elapsed_span = undefined, seconds_remaining = 0;
	try {
		seconds_elapsed_span = document.getElementById("seconds_elapsed_span");
		seconds_elapsed = parseInt(seconds_elapsed_span.innerHTML);
		if (seconds_elapsed < 0) throw "seconds_elapsed must be an integer no smaller than 0.";
		seconds_elapsed += 1;
		seconds_elapsed_span.innerHTML = seconds_elapsed;
	}
	catch(exception) {
		console.log("An exception to expected functioning occurred in increment_seconds_elapsed(): " + exception);
		return 0;
	}
}
...
/**
 * Make the START button invisible as soon as it is clicked and for the duration of the (indefinitely executing) Monte Carlo dart-throwing simulation.
 * 
 ...
 * 
 * Set the value enclosed by the span element whose id is "seconds_elapsed_span" to 0.
 * 
...
function initialize_application() {
	...
	let seconds_elapsed_span, red_pixel_count_span, blue_pixel_count_span, pi_approximation_span, timestamp, message, console_div;
	try {
		...
		seconds_elapsed_span.innerHTML = 0; 
...
/**
 * Call plot_random_pixel_on_square_canvas(simulation) once per second (for an indefinitely long period of time).
 * 
 * Each time plot_random_pixel_on_square_canvas(simulation) is called, increment seconds_elapsed by 1.
 * 
 ...
 * Assume that this function is called in response to the START button being clicked.
 * 
 * Throw one random pixel-sized dart onto the square canvas per second for an indefinitely long time period.
 * 
...
function start_monte_carlo_simulation() {
	...
	hide_start_button();
	simulation = setInterval( function() { plot_random_pixel_on_square_canvas(simulation); }, 1000); // The plot_random_pixel_on_square_canvas(simulation) function is called once every 1000 milliseconds.
}